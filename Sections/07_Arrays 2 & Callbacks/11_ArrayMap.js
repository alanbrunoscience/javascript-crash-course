 ...
 * 
 * Notice that you always get back an array containing the SAME NUMBER OF ITEMS compared to the original array, but 
 * every item has most likely undergone some transformation.
 * 
 * In the first example, the transformation is that we multiply every number by 2. In the second example, the 
 * transformation is that we call .toUpperCase() on every item.
 * 
...
/**
 * If you forget to return from inside the callback function, you will end up with the following array: 
 * [undefined, undefined]. That's because, for every item in the original array (["sam", "Alex"]), you're returning 
 * undefined so the end result will be [undefined, undefined].
 * 
