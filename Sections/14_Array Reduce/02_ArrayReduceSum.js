 ...
 * Array reduce: sum
 * 
 * - Time to take a look at the code! In this lesson, we'll focus on one use case of reduce: calculating the sum.
 * Thus, reducing an array of numbers into its sum.
 * 
 ...
 * Initial Value
 * 
 * - The .reduce() method accepts 2 parameters: reducer and initialValue (not to be confused by the 2 parameters
 * of the reducer which are total and current). We explained the reducer above. The initialValue is the value we
 * give to the total (or accumulator) the first time the callback runs.
 * 
 * - Passing '0' as initialValue is the same as declaring 'let sum = 0' when using '.forEach()' to calculate the
 * 'sum'. It's the starting value that we use to be able to calculate the 'sum'.
 * 
 * - JavaScript will automatically take your 'initialValue' and pass it to the 'total' argument in the reducer the
 * first time that callback runs.
 * 
 ...
 * 
 * - The 'total' (also called 'accumulator') keeps track of the result of the reduce method. For example, when 
 * calculating the sum, it keeps track of the sum, step by step.
 * 
